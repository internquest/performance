import React from 'react'

const OurPillars = () => {
  return (
    <section
  className="c-section c-section--light c-section--card-cyan-highlight c-cards c-cards--4"
  data-animate-section=""
  data-nav-section="light"
  style={{ opacity: 1 }}
>
  <div className="wrap wrap--max">
    <div className="c-grid">
      <div className="col-span-10 md:col-span-12 md:col-start-2 lg:col-span-8 lg:col-start-4 text-center">
        <h2 className="ts-2" data-animate-title="" style={{}}>
          <div className="title-line">
            <div className="title-line-inner" style={{}}>
              Our <span className="title-span" style={{ "--span-x": 1 }}>pillars</span>
            </div>
          </div>
        </h2>
        <div
          className="ts-p"
          data-split-articles=""
          data-animate-paragraph=""
          data-animate-delay="200"
          style={{}}
        >
          <p style={{}}>
            <div className="line">
              <div className="line-inner" style={{}}>
                At PerformanSe, “Trust in people” is a value upheld and woven into the practice of
                our tools. The
              </div>
            </div>
            <div className="line">
              <div className="line-inner" style={{}}>
                objective assessment of the person lies at the heart of our approach, and
                necessarily combines
              </div>
            </div>
            <div className="line">
              <div className="line-inner" style={{}}>
                with the human aspect, thus contributing to these assessment approaches and
                participating in an
              </div>
            </div>
            <div className="line">
              <div className="line-inner" style={{}}>ecology of the mind.</div>
            </div>
          </p>
        </div>
      </div>
      <div
        className="col-span-8 col-start-2 md:col-span-14 c-card-items"
        data-animate-items=""
        data-animate-delay="400"
        style={{}}
      >
        <div className="c-card-item c-card-item--8" style={{}}>
          <div className="c-card-item-title">
            <h3>The scientific approach</h3>
          </div>
          <div className="c-card-item-text ts-p">
            <p>
              Using reference scientific models, compliance with international standards,
              validation by university laboratories and certified R&D projects, PerformanSe tests
              are the most accurate on the market.
            </p>
          </div>
          <div className="c-card-item-icon">
            <svg
              width="42"
              height="42"
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M13.9655 6C11.787 6 10 7.79133 10 10.0271V33.785L12.6145 31.4797C13.007 31.1336 13.6 31.1486 13.9745 31.5142L17.138 34.6025L20.3014 31.5142C20.6899 31.1349 21.31 31.1349 21.6985 31.5142L24.862 34.6025L28.0257 31.5142C28.4001 31.1486 28.9931 31.1336 29.3856 31.4797L32 33.785V10.0271C32 7.79131 30.2131 6 28.0345 6H13.9655ZM8 10.0271C8 6.71005 10.6593 4 13.9655 4H28.0345C31.3408 4 34 6.71007 34 10.0271V36C34 36.3931 33.7697 36.7497 33.4115 36.9114C33.0532 37.0732 32.6334 37.01 32.3386 36.7501L28.7588 33.5935L25.5606 36.7156C25.1721 37.0948 24.552 37.0948 24.1635 36.7156L21 33.6272L17.8365 36.7156C17.448 37.0948 16.8279 37.0948 16.4394 36.7156L13.2413 33.5935L9.66137 36.7501C9.36654 37.01 8.94677 37.0732 8.58852 36.9114C8.23027 36.7497 8 36.3931 8 36V10.0271ZM26.0475 12.9462C26.4404 13.3343 26.4443 13.9675 26.0562 14.3604L20.8148 19.6668C20.6269 19.8571 20.3707 19.9641 20.1034 19.9641C19.836 19.9641 19.5798 19.857 19.3919 19.6668L16.7713 17.0136C16.3832 16.6206 16.3871 15.9875 16.7801 15.5994C17.173 15.2113 17.8062 15.2152 18.1943 15.6081L20.1034 17.5411L24.6333 12.9549C25.0214 12.562 25.6546 12.5581 26.0475 12.9462ZM14.6207 25.1081C14.6207 24.5559 15.0684 24.1081 15.6207 24.1081H26.3793C26.9316 24.1081 27.3793 24.5559 27.3793 25.1081C27.3793 25.6604 26.9316 26.1081 26.3793 26.1081H15.6207C15.0684 26.1081 14.6207 25.6604 14.6207 25.1081Z"
                fill="#102458"
              ></path>
            </svg>
          </div>
        </div>
        <div className="c-card-item c-card-item--8" style={{}}>
          <div className="c-card-item-title">
            <h3>The scientific approach</h3>
          </div>
          <div className="c-card-item-text ts-p">
            <p>
              Using reference scientific models, compliance with international standards,
              validation by university laboratories and certified R&D projects, PerformanSe tests
              are the most accurate on the market.
            </p>
          </div>
          <div className="c-card-item-icon">
            <svg
              width="42"
              height="42"
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M13.9655 6C11.787 6 10 7.79133 10 10.0271V33.785L12.6145 31.4797C13.007 31.1336 13.6 31.1486 13.9745 31.5142L17.138 34.6025L20.3014 31.5142C20.6899 31.1349 21.31 31.1349 21.6985 31.5142L24.862 34.6025L28.0257 31.5142C28.4001 31.1486 28.9931 31.1336 29.3856 31.4797L32 33.785V10.0271C32 7.79131 30.2131 6 28.0345 6H13.9655ZM8 10.0271C8 6.71005 10.6593 4 13.9655 4H28.0345C31.3408 4 34 6.71007 34 10.0271V36C34 36.3931 33.7697 36.7497 33.4115 36.9114C33.0532 37.0732 32.6334 37.01 32.3386 36.7501L28.7588 33.5935L25.5606 36.7156C25.1721 37.0948 24.552 37.0948 24.1635 36.7156L21 33.6272L17.8365 36.7156C17.448 37.0948 16.8279 37.0948 16.4394 36.7156L13.2413 33.5935L9.66137 36.7501C9.36654 37.01 8.94677 37.0732 8.58852 36.9114C8.23027 36.7497 8 36.3931 8 36V10.0271ZM26.0475 12.9462C26.4404 13.3343 26.4443 13.9675 26.0562 14.3604L20.8148 19.6668C20.6269 19.8571 20.3707 19.9641 20.1034 19.9641C19.836 19.9641 19.5798 19.857 19.3919 19.6668L16.7713 17.0136C16.3832 16.6206 16.3871 15.9875 16.7801 15.5994C17.173 15.2113 17.8062 15.2152 18.1943 15.6081L20.1034 17.5411L24.6333 12.9549C25.0214 12.562 25.6546 12.5581 26.0475 12.9462ZM14.6207 25.1081C14.6207 24.5559 15.0684 24.1081 15.6207 24.1081H26.3793C26.9316 24.1081 27.3793 24.5559 27.3793 25.1081C27.3793 25.6604 26.9316 26.1081 26.3793 26.1081H15.6207C15.0684 26.1081 14.6207 25.6604 14.6207 25.1081Z"
                fill="#102458"
              ></path>
            </svg>
          </div>
        </div>
        <div className="c-card-item c-card-item--8" style={{}}>
          <div className="c-card-item-title">
            <h3>The scientific approach</h3>
          </div>
          <div className="c-card-item-text ts-p">
            <p>
              Using reference scientific models, compliance with international standards,
              validation by university laboratories and certified R&D projects, PerformanSe tests
              are the most accurate on the market.
            </p>
          </div>
          <div className="c-card-item-icon">
            <svg
              width="42"
              height="42"
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M13.9655 6C11.787 6 10 7.79133 10 10.0271V33.785L12.6145 31.4797C13.007 31.1336 13.6 31.1486 13.9745 31.5142L17.138 34.6025L20.3014 31.5142C20.6899 31.1349 21.31 31.1349 21.6985 31.5142L24.862 34.6025L28.0257 31.5142C28.4001 31.1486 28.9931 31.1336 29.3856 31.4797L32 33.785V10.0271C32 7.79131 30.2131 6 28.0345 6H13.9655ZM8 10.0271C8 6.71005 10.6593 4 13.9655 4H28.0345C31.3408 4 34 6.71007 34 10.0271V36C34 36.3931 33.7697 36.7497 33.4115 36.9114C33.0532 37.0732 32.6334 37.01 32.3386 36.7501L28.7588 33.5935L25.5606 36.7156C25.1721 37.0948 24.552 37.0948 24.1635 36.7156L21 33.6272L17.8365 36.7156C17.448 37.0948 16.8279 37.0948 16.4394 36.7156L13.2413 33.5935L9.66137 36.7501C9.36654 37.01 8.94677 37.0732 8.58852 36.9114C8.23027 36.7497 8 36.3931 8 36V10.0271ZM26.0475 12.9462C26.4404 13.3343 26.4443 13.9675 26.0562 14.3604L20.8148 19.6668C20.6269 19.8571 20.3707 19.9641 20.1034 19.9641C19.836 19.9641 19.5798 19.857 19.3919 19.6668L16.7713 17.0136C16.3832 16.6206 16.3871 15.9875 16.7801 15.5994C17.173 15.2113 17.8062 15.2152 18.1943 15.6081L20.1034 17.5411L24.6333 12.9549C25.0214 12.562 25.6546 12.5581 26.0475 12.9462ZM14.6207 25.1081C14.6207 24.5559 15.0684 24.1081 15.6207 24.1081H26.3793C26.9316 24.1081 27.3793 24.5559 27.3793 25.1081C27.3793 25.6604 26.9316 26.1081 26.3793 26.1081H15.6207C15.0684 26.1081 14.6207 25.6604 14.6207 25.1081Z"
                fill="#102458"
              ></path>
            </svg>
          </div>
        </div>
        <div className="c-card-item c-card-item--8" style={{}}>
          <div className="c-card-item-title">
            <h3>The scientific approach</h3>
          </div>
          <div className="c-card-item-text ts-p">
            <p>
              Using reference scientific models, compliance with international standards,
              validation by university laboratories and certified R&D projects, PerformanSe tests
              are the most accurate on the market.
            </p>
          </div>
          <div className="c-card-item-icon">
            <svg
              width="42"
              height="42"
              viewBox="0 0 42 42"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M13.9655 6C11.787 6 10 7.79133 10 10.0271V33.785L12.6145 31.4797C13.007 31.1336 13.6 31.1486 13.9745 31.5142L17.138 34.6025L20.3014 31.5142C20.6899 31.1349 21.31 31.1349 21.6985 31.5142L24.862 34.6025L28.0257 31.5142C28.4001 31.1486 28.9931 31.1336 29.3856 31.4797L32 33.785V10.0271C32 7.79131 30.2131 6 28.0345 6H13.9655ZM8 10.0271C8 6.71005 10.6593 4 13.9655 4H28.0345C31.3408 4 34 6.71007 34 10.0271V36C34 36.3931 33.7697 36.7497 33.4115 36.9114C33.0532 37.0732 32.6334 37.01 32.3386 36.7501L28.7588 33.5935L25.5606 36.7156C25.1721 37.0948 24.552 37.0948 24.1635 36.7156L21 33.6272L17.8365 36.7156C17.448 37.0948 16.8279 37.0948 16.4394 36.7156L13.2413 33.5935L9.66137 36.7501C9.36654 37.01 8.94677 37.0732 8.58852 36.9114C8.23027 36.7497 8 36.3931 8 36V10.0271ZM26.0475 12.9462C26.4404 13.3343 26.4443 13.9675 26.0562 14.3604L20.8148 19.6668C20.6269 19.8571 20.3707 19.9641 20.1034 19.9641C19.836 19.9641 19.5798 19.857 19.3919 19.6668L16.7713 17.0136C16.3832 16.6206 16.3871 15.9875 16.7801 15.5994C17.173 15.2113 17.8062 15.2152 18.1943 15.6081L20.1034 17.5411L24.6333 12.9549C25.0214 12.562 25.6546 12.5581 26.0475 12.9462ZM14.6207 25.1081C14.6207 24.5559 15.0684 24.1081 15.6207 24.1081H26.3793C26.9316 24.1081 27.3793 24.5559 27.3793 25.1081C27.3793 25.6604 26.9316 26.1081 26.3793 26.1081H15.6207C15.0684 26.1081 14.6207 25.6604 14.6207 25.1081Z"
                fill="#102458"
              ></path>
            </svg>
          </div>
        </div>
        {/* Add other card items similarly */}
      </div>
    </div>
  </div>
</section>

  )
}

export default OurPillars